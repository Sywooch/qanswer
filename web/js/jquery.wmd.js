...
Attacklab.wmdBase = function(){
	...
	// entering links.
	var imageDialogText = wmd_options.imageDialogText || "<p style='margin-top: 0px'><b>输入图像地址.</b></p><p>可以添加一个标题（title）属性.</p><p>例子:<br />http://wmd-editor.com/images/cloud1.jpg   \"标题\"</p>";
	var linkDialogText = wmd_options.linkDialogText || "<p style='margin-top: 0px'><b>输入网址.</b></p><p>可以添加一个标题（title）属性.</p><p>例子:<br />http://wmd-editor.com/   \"标题\"</p>";
	...
	wmd.editor = function(previewRefreshCallback){
		...
		var makeSpritedButtonRow = function(){
			...
			function addSpacer(i) {
			    ...
			    spacer.className = "wmd-spacer " + ("wmd-spacer" + i);
//			    spacer.id = "wmd-spacer" + i;
			...
			var quoteButton = addButton("wmd-quote-button", "块引用 <blockquote> Ctrl+Q", command.doBlockquote);
			var codeButton = addButton("wmd-code-button", "代码 <pre><code> Ctrl+K", command.doCode);
			var imageButton = addButton("wmd-image-button", "插入图像 <img> Ctrl+G", function(chunk, postProcessing, useDefaultText) {
				return command.doLinkOrImage(chunk, postProcessing, true);
			});
			
			var spacer2 = addSpacer(2);

			var olistButton = addButton("wmd-olist-button", "编号列表 <ol> Ctrl+O", function(chunk, postProcessing, useDefaultText) {
				command.doList(chunk, postProcessing, true, useDefaultText);
			});
			var ulistButton = addButton("wmd-ulist-button", "项目列表 <ul> Ctrl+U", function(chunk, postProcessing, useDefaultText) {
				command.doList(chunk, postProcessing, false, useDefaultText);
			});
			var headingButton = addButton("wmd-heading-button", "标题 <h1>/<h2> Ctrl+H", command.doHeading);
			var hrButton = addButton("wmd-hr-button", "水平线 <hr> Ctrl+R", command.doHorizontalRule);
			var spacer3 = addSpacer(3);
			
			var undoButton = addButton("wmd-undo-button", "撤销 - Ctrl+Z");
...
(function($) {
	...
	$.fn.TextAreaResizer = function() {
		return this.each(function() {
			i = $(this).addClass("processed");
			g = null;
			$(this).after($('<div class="grippie"></div>').bind("mousedown", {
				el: this
			},
			j));
			var k = $("div.grippie", $(this).parent())[0];
			k.style.marginRight = (k.offsetWidth - $(this)[0].offsetWidth) + "px"
		})
	};
	function j(k) {
		i = $(k.data.el);
		...
		g = i.height() - c;
		i.css("opacity", 0.25);
		$(document).mousemove(e).mouseup(a);
		return false
	}
	function e(m) {
		var k = d(m).y;
	...
	function a(k) {
		$(document).unbind("mousemove", e).unbind("mouseup", a);
		i.css("opacity", 1);
		i.focus();
		i = null;
		g = null;
